...
const Books = () => {
  ...
  useEffect(() => {
    if (booksError || productsError || articlesError) {
      console.error("Query errors detected:", { booksError, productsError, articlesError });
      setHasError(true);
  ...
  const carouselItems = allBooks.length > 0 
    ...
    : [
        { id: 1, title: "Nigerian Heritage", image: getDefaultImage(), type: "book" as const },
        ...
        { id: 3, title: "Cultural Journeys", image: "https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=400&h=600&fit=crop", type: "book" as const },
      ];
  ...
  return (
    <Layout>
      <div className="py-12 md:py-20">
        <h1 className="section-title text-center mb-12 animate-fade-in">BOOKS & PUBLISHING</h1>
        ...
        {booksLoading ? (
          ...
          <>
            ...
            <section className="mx-auto max-w-6xl px-6 mb-16">
              <h2 className="text-xl font-serif text-center mb-12">Our Book Collection</h2>
                      ...
                      <CardContent className="p-4">
                        ...
                        {book.releaseDate && (
                          <div className="flex items-center gap-1 text-xs text-muted-foreground mt-1">
                            ...
                            Published {new Date(book.releaseDate).getFullYear()}
                          </div>
                        )}
                        {book.description && (
                        ...
                        {externalLinks.length > 0 && (
                          <div className="mt-3 pt-3 border-t">
                            <p className="text-xs font-medium mb-1">Available at:</p>
            ...
            {products.length > 0 && (
              <section className="mx-auto max-w-6xl px-6 mb-16">
                <h2 className="text-xl font-serif text-center mb-12">Purchase Books</h2>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {products.map((product) => (
                    <Link key={product.id} to={`/store/${product.slug}`}>
                      <Card className="overflow-hidden group hover:shadow-lg transition-shadow">
                        ...
                        <CardContent className="p-4">
                          <h3 className="font-serif font-medium">{product.title}</h3>
                          {product.category && (
          ...
          <section className="mx-auto max-w-2xl px-6 text-center mb-12">
            <p className="text-muted-foreground">
              Our book collection is being prepared. Check back soon for amazing Nigerian literature!
            </p>
          </section>
        )}
        ...
        <section className="mx-auto max-w-2xl px-6 mb-12">
          <h2 className="text-xl font-serif text-center mb-12">Publishing News</h2>
            ...
            {articlesLoading ? (
              ...
              <>
                <article className="border border-foreground/20 p-6 text-center animate-slide-up">
                  <p className="text-sm font-medium text-accent mb-2">Publishing (March 2025)</p>
                  <p className="text-sm text-foreground/80">New Nigerian Authors Book Series Launch Announced</p>
                </article>
                <article className="border border-foreground/20 p-6 text-center animate-slide-up" style={{ animationDelay: "0.1s" }}>
                  <p className="text-sm font-medium text-accent mb-2">Book Awards (February 2025)</p>
                  <p className="text-sm text-foreground/80">BAUHAUS Publishing Wins African Literature Award</p>
                </article>
              </>
            )}
          </div>
